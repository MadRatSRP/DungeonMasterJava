...
//to do:
//------
//redo editor help files and get new screen shots
//get/make more mon sounds,ress/reinc sound(s),hero death,cast potion,zokathra,drain,mon heal sound,other new spells,stormfeed,hit
//?fix activate/deactivate alcove switches to work better with switch counters -> move count stuff to sep method or something
//? - maybe get rid of switch counts for everything but add them to wall and/or floorswitches? (single and/or multiple?)
//?maybe change how pits and tels work: telefrag and pitfrag (whatever already there dies -> this is how DM did it apparently)
//?maybe change falling into pit onto teleport do damage (so infinite loop is avoided by death)
//?may be better scale pic before applying darken filter, save scaled and darkened version in memory array
//?maybe a good weapon/item? have stormbringer and hellion but no goodly stuff really
//?maybe change switched teleport to have actual floorswitch
//?maybe change poison missle and/or ven cloud so doesn't do damage, only poisons

//------------------------------
//possible improvements/changes:
//------------------------------
//random decoration button in editor (current level only, checks for walls next to floors, 20% randomly changed to a dec facing the floor)
//?something special for hellion (#95)
// -- if wear it long enough (don't bother to save counter?), become enslaved by it message.setMessage("The Hellion flares brightly and "+name+" is racked (sp?) by unbearable agony.",5); //gripped by? consumed with?
//  -- message.setMessage("The Hellion pulses briefly with a deep red light. "+name+" feels dizzy.",5); <- have this every so often when count is increasing
//  -- change hellion picture after activation -> animated gif
//  -- grants some special powers but also weaknesses:
//   -- no stamina anymore, don't need eat/drink, can't be healed (health and mana) by resting - only potions, immune to poison, weak against fire, can't be silenced or slowed or drained or weakened in any way
//   -- if die, generate a deth knight or a demon? maybe just consume soul? maybe nothing special, just no longer affected by it? maybe hellion destroyed?
//   -- maybe other effects too?
//implement better music system (vary music depending on mons/traps/areas)?
//torches in sconces could be burning and giving off light
//optimize drawdungeon - see notebook
//maybe do other optimizations (like main arraylist for mons, and each mapobject has own array too - faster?)
//teleport that randomly teleports you to a couple of places.
//a monster generator that spawns a randomly chosen monster.
//?modify attack routine (mon and party) - if 2 mons/heroes in op. corners, make so can hit both
//launcher switch counter
//miss more, hit less hard when is dark
//weapon types (blunt hurts skeleton, etc.) and spell weaknesses (giggler weak against poison, some mons resist fireballs)
//put boards/chunks metal after door broken
//add sound on/off to options dialog - maybe music volume control?
//more spells
// - create food spell (like manna created by the gods)
// - monster stat boosters
// - web (once step in, can't move or swing weapon until wears off - can still cast?)
//   -  fireballs destroy
// - wall of fire
//   - maybe wall of fire is an actual mapobject wall, not a spell? - switches activate/deactivate
//   - could be mapobject swapped in by a spell (old mapobject must be floor w/o mons -> saved somewhere)
// - aura of ra -> perfect shield and/or attack all sides?
// - reflector(power determines duration and max power spell can reflect)
// - freeze, fear
//update monster ai (see notebook)
//multiplayer option - clients could play as monsters...
// -- certain spells available (maybe none)
// -- would need slightly different interface, could be fun
//assign random weak items to character created (based somewhat on levels selected, if applicable)
// -- maybe pick some (weaker) items? buy them with special starting money?
//add effects to weapon strikes (slow, drain, etc.)
//food/water consumption - dif chars have dif rates (high,normal,low) - big,bulky chars need eat/drink more
//do arrows like baldur's gate -> 3 quiver slots, groups of up to 20 arrows/rocks/etc?
//make dragon armor (or ven armor?) resist ven clouds
//add a type to spells,weapons,armor (fire,poison,lightning,ice,etc) - some mons resist/use certain types
//NPCs
//ai improvements
// -- use stairs better somehow, also dodge projs and go around corners
//only one sideview, other is mirrored -> this is actually slower, but may use less memory
//maybe center pics in sidedwall2, adjust pics to place dif, no need of xadjust,yadjust arrays??
//change fountain,altar,and onealcove to extend sidedwall2, maybe also alcove?

//---------------
//ideas/thoughts:
//---------------
//demon char that doesn't eat/drink but must kill something to survive (or vampire char)
//make warhammers, ?perhaps a special hammer of thor that can take out walls - but not borderwalls (any wall that can pass immaterial? maybe only certain special walls with visible cracks?)
// -- put rocks/boulders on new floor after wall broken
//?poisoned fountains?
//swords - shieldbreaker,stonecutter,woundhealer,dragonslicer,sightblinder,doomgiver?
//the blue sword gonturan - only usable by woman hero - best sword in game - would need male/female flag, or could be in a mirror with aerin...
//tarrant's coldfire blade
...
public class dmnew extends JFrame implements ActionListener, MouseListener, ComponentListener, Runnable {//, ControllerListener {
    ...
    static boolean NODARK;// = true;
    static boolean TEXTANTIALIAS = true;
    static boolean AUTOMAP, SHOWPARTYMAP;
    static boolean PLAYFOOTSTEPS = true;
    ...
    //static String endmusic = "fuseend.mid";//music for multiple endings
    ...
    static boolean herolook, allowswap;//,eventflag;
    static boolean nomovement; //mons,projs,& heroes don't change (during game load/save & mirrorsheet)
    static boolean mapchanging;
    static boolean cloudchanging;
    static boolean fluxchanging;
    static Collection mapstochange = new ArrayList(5);//Vector(3,1);
    static Collection cloudstochange = new ArrayList(3);//Vector(2,1);
    static HashMap fluxcages = new HashMap(10);//Vector(2,1);
    ...
    //static Component mcpglue = Box.createVerticalGlue();
    ...
    static boolean needdrawdungeon = true;
    int spellready = 0;
    int weaponready = 0;
    ...
    //static Image[][] mondarkpic = new Image[29][12];
    //static Image[][] mondarkpic;
    static HashMap mondarkpic = new HashMap();
    //static int maxmonnum = 1;
    static final int[][] cagex = {{0, 0, 384}, {0, 64, 328}, {0, 120, 298}, {0, 148, 278}};
    static final int[] cagey = {0, 22, 50, 62};
    //static Vector animimgs = new Vector(2,1);
    //static Collection animimgs = new ArrayList(2);//Vector(2,1);
    ...
    //static MusicThread music;
    //static FileDialog chooser;
    static JFileChooser chooser;
    static DMMap dmmap = null;
    OptionsDialog optionsdialog;
    
    static GraphicsDevice currentDevice;
    static DisplayMode originalMode, currentMode;
    //static DisplayMode[] modeList;
    static boolean displayModeChanged = false, fullscreen = true;
    ...
    ImageTilePanel imagePane = new ImageTilePanel("bluerock.gif");
    ...
    static boolean mirrorback, nomirroradjust;
    ...
    /*
        static final String[] MONSTERSOUNDS = {
                "mummy.wav","spider.wav","giggler.wav","rockmon.wav","slime.wav",null,"mummy.wav",
                null,"swing.wav","worm.wav",null,"waterel.wav",null,"roar.wav","swing.wav",null,
                "couatyl.wav","rockmon.wav","slime.wav","scorpion.wav",null,"swing.wav","spider.wav","swing.wav",
                null,"roar.wav",null,"demonlord.wav" };
        */
    ...
    static final Integer IFORWARD = new Integer(0);
    static final Integer IBACK = new Integer(1);
    static final Integer ILEFT = new Integer(2);
    static final Integer IRIGHT = new Integer(3);
    static final Integer ITURNLEFT = new Integer(4);
    static final Integer ITURNRIGHT = new Integer(5);
    ...
    static final int DOORHIT = 3; //door bashing head
    static final int STORMHIT = 4; //stormbringer feeds
    static final int PROJWEAPONHIT = 5;
    static final int FUSEHIT = 6; //fusing chaos
    static final int DRAINHIT = 7; //drain life
    static final int[][] SYMBOLCOST = {
    ...
    private dmnew() {
        ...
        DisplayMode[] modeList = currentDevice.getDisplayModes();
        ...
        //mondeathpic = tk.createImage("mondeath.gif");
        ...
        resreincpic = tk.createImage("resreinc.gif");
        ...
        //music = new MusicThread("Music");
        //music.setSpecial("Music"+File.separator+"dawn.mid");
        ...
        hpanel.setMinimumSize(new Dimension(420, 130));
        hpanel.setMaximumSize(new Dimension(420, 130));
        hpanel.setPreferredSize(new Dimension(420, 130));
        ...
        toppanel.add(hpanel);
        toppanel.setPreferredSize(new Dimension(662, 134));
        toppanel.setMaximumSize(new Dimension(662, 134));
        ...
        final Dimension centerdim = new Dimension(448, 326);
        final Dimension centerdim2 = new Dimension(452, 330);
        dview = new DungView();
        ...
        freezelabelpan.setSize(448, 326);
        ...
        loadinglabelpan.setSize(448, 326);
        ...
        //endiconlabelpan.setLayout(new BoxLayout(endiconlabelpan,BoxLayout.Y_AXIS));
        endiconlabelpan.setSize(448, 326);
        ...
        //animlabelpan.setLayout(new BoxLayout(animlabelpan,BoxLayout.Y_AXIS));
        animlabelpan.setSize(448, 326);
        ...
        if (fullscreen && currentDevice.isFullScreenSupported()) {
            ...
            if (currentDevice.isDisplayChangeSupported()) {
                ...
                //        System.out.println(i+" - " + modeList[i].getWidth() + " x " + modeList[i].getHeight() + " x " + modeList[i].getBitDepth() + " @ " + modeList[i].getRefreshRate()+"hz");
                //}
                for (int i = modeList.length - 1; i >= 0; i--) {
                    //if (modeList[i].getWidth()==800 && modeList[i].getHeight()==600) {
                    if (modeList[i].getWidth() == trywidth && modeList[i].getHeight() == tryheight && (trybitdepth == 0 || modeList[i].getBitDepth() == trybitdepth) && (tryrefresh == 0 || modeList[i].getRefreshRate() == tryrefresh)) {
                        currentMode = modeList[i];
                        break;
                    }
                }
                if (currentMode != null) {
        ...
        try {
            ...
            for (int i = 0; i < nummons; i++) {
                ...
                if (monnum < 28) tempmon = new Monster(monnum, si.readInt(), si.readInt(), si.readInt());
                    ...
                    tempmon = new Monster(monnum, si.readInt(), si.readInt(), si.readInt(), si.readUTF(), si.readUTF(), si.readUTF(), si.readUTF(), si.readBoolean(), si.readBoolean(), si.readBoolean(), si.readBoolean());
                //tempmon = new Monster(si.readInt(),si.readInt(),si.readInt(),si.readInt());
            ...
            for (int i = 0; i < numprojs; i++) {
                ...
                type = si.readBoolean();
                if (type)
                    tempproj = new Projectile((Item) si.readObject(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readBoolean(), si.readBoolean());
                else
                    tempproj = new Projectile((Spell) si.readObject(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readInt(), si.readBoolean(), si.readBoolean());
            ...
            //try {
            //java.io.OutputStream out = new java.io.FileOutputStream("dmnewerror.log");
            ...
            //ImageTracker.waitForID(2);//spell pics
            //ImageTracker.waitForID(4);//item pics
            Item.doFlaskBones();
    ...
    public void startNew() {
        ...
        hpanel.add(hero[0]);
        //hero[0].offscreen = createImage(100,124);
        //hero[0].offg = hero[0].offscreen.getGraphics();
        //hero[0].setCursor(dmcursor);
    ...
    public static void main(String[] args) {
        ...
        try {
            FileInputStream in = new FileInputStream("gamefont.ttf");
        ...
        if (cfg.exists()) {
            try {
                ...
                r.readLine();//skip display message
                String temps = r.readLine();
                fullscreen = (Integer.parseInt(temps.substring(11)) != 0);
                trywidth = Integer.parseInt(r.readLine());
                tryheight = Integer.parseInt(r.readLine());
                trybitdepth = Integer.parseInt(r.readLine());
                tryrefresh = Integer.parseInt(r.readLine());
                temps = r.readLine();
                while (temps != null) {
                    temps = temps.toLowerCase();
                    if (temps.startsWith("speed")) {
                    ...
                    else if (temps.startsWith("antialias"))
                        TEXTANTIALIAS = (Integer.parseInt(temps.substring(10)) != 0);
                    else if (temps.startsWith("footsteps"))
                        PLAYFOOTSTEPS = (Integer.parseInt(temps.substring(10)) != 0);
        ...
        WindowListener l = new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                //confirm dialog
                //int returnval = JOptionPane.showConfirmDialog(frame,"Exit Game. Are You Sure?","Quit?",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
        ...
        public void paint(Graphics g) {
                //662,582 default frame size
                double scalex = getSize().width/662.0, scaley = getSize().height/582.0;
                ((Graphics2D)g).setTransform(java.awt.geom.AffineTransform.getScaleInstance(scalex,scaley));
                super.paint(g);
                
                //this doesn't quite work, but has potential. swing stuff and all mouse things would need work.
        }
        */
    ...
    public void run() {
        ...
        while (!didquit) {
            while (endcounter < 20) {
                if (nomovement || (needdrawdungeon && !sheet && !mappane.isVisible())) {
                } //do nothing
                else {
                    if (!clickqueue.isEmpty()) {
                        ...
                        if (what.equals("s")) {
                            ...
                            //else if (loadGame(false,(numheroes==0))) {
                            //        hpanel.repaint();
                            //        changeCursor();
                            //}
                            //else {
                            ...
                            //show options
                            //int selectedValue = (new OptionsDialog(frame,(numheroes==0))).getValue();
                            ...
                            if (selectedValue == OptionsDialog.LOAD) {
                                ...
                                if (selectedValue == OptionsDialog.NEWCUST) {
                                                        ...
                                                        */
                                    chooser.setCurrentDirectory(new File("Dungeons"));
                    ...
                    if (bonesflag > -1) {
                        ...
                        for (int i = 0; i < numtimes; i++) {
                            if (magicvision > 0) {
                                magicvision--;
                                if (magicvision == 0) needredraw = true;
                            }
                            if (dispell > 0) {
                                dispell--;
                                if (dispell == 0) needredraw = true;
                            }
                            if (floatcounter > 0) {
                                floatcounter--;
                                if (floatcounter == 0) {
                    ...
                    if (counter % 2 == 0) {
                        ...
                        if (!walkqueue.isEmpty()) {
                            if (walkdcount == walkdelay) {
                                int i = 0;
                                walkdelay = 2;//was 3,10,6
                                if (slowcount > 0) {
                                    boolean speedboots = true;
                                    ...
                                    if (!hero[i].isdead && (hero[i].feet == null || hero[i].feet.number != 187)) {
                                        if (hero[i].hurtfeet || hero[i].hurtlegs || hero[i].load > hero[i].maxload)
                                            walkdelay = 8;
                                        else if (hero[i].hurttorso || hero[i].load > hero[i].maxload * 3 / 4)
                                            walkdelay = 4;
                                    }
                                    i++;
                                }
    ...
    public static void shutDown() {
        ...
        try {
            //write out config file
            PrintWriter w = new PrintWriter(new FileWriter("dmnew.cfg"));
            w.println("; Display Settings. Default 800x600, 0(depth), 0(refresh) (0 for depth and refresh means use best, but this doesn't work for some. other possibles are 8,16,24,or 32 and 60,72,85,etc).");
            if (currentDevice.getFullScreenWindow() != null) {
                w.println("fullscreen 1");
                w.println("" + currentMode.getWidth());
                w.println("" + currentMode.getHeight());
                w.println("" + trybitdepth);
                w.println("" + tryrefresh);
            } else {
                w.println("fullscreen 0");
                w.println("" + frame.getSize().width);
                w.println("" + frame.getSize().height);
                w.println("" + trybitdepth);
                w.println("" + tryrefresh);
            }
            w.println("; Game speed.  Values range from 10 (fastest) to 100 (slowest).  Default 45.");
            ...
            w.println("; Show the party on the automap.  0 to turn off.");
            w.println("showparty " + (SHOWPARTYMAP ? 1 : 0));
            w.println("; Boost the dungeon display brightness.  Values range from 0 (none) to 32 (brightest).  Default 0.");
    ...
    public void showCenter(JComponent c) {
    ...
    public static void updateDark() {
        leveldark = leveldarkfactor[level] + numillumlets * 70;
        if (leveldark > 255) leveldark = 255;
        ...
        if ((leveldir[level] != null && !leveldir[level].equals(Wall.currentdir)) || (leveldir[level] == null && !Wall.currentdir.equals("Maps"))) {
            ...
            boolean backexists = false, trackexists = false;
            ...
            for (int x = 0; x < mapwidth; x++) {
                for (int y = 0; y < mapheight; y++) {
                    ...
                    else if (DungeonMap[level][x][y].mapchar == ']')
                        ((OneAlcove) DungeonMap[level][x][y]).redoAlcovePics();
                    else if (DungeonMap[level][x][y].mapchar == 'w')
                        ((Writing2) DungeonMap[level][x][y]).redoWritPics();
                    else if (DungeonMap[level][x][y].mapchar == 'f')
                        ((Fountain) DungeonMap[level][x][y]).redoFountainPics();
        ...
        while (index < dmprojs.size()) {
            ...
            if (tempp.isending) {
                ...
                if (tempp.level == level) {
                    int xdist = tempp.x - partyx;
                    if (xdist < 0) xdist *= -1;
                    int ydist = tempp.y - partyy;
                    if (ydist < 0) ydist *= -1;
    ...
    public static void stopSounds(boolean abrupt) {
        LoopSound sound;
    ...
    public void resurrection(int i) {
        ...
        if (!foundsub) {
            hero[i].subsquare = numheroes;
            heroatsub[numheroes] = i;
        }
        formation.addNewHero();
        hero[i].defense -= hero[i].defenseboost;
        hero[i].defenseboost = 0;
        hero[i].magicresist -= hero[i].magicresistboost;
        hero[i].magicresistboost = 0;
        ...
        int drain = randGen.nextInt(hero[i].maxstamina / 50 + 1);
        if (drain > 10) drain = 10;
        hero[i].maxstamina -= drain;
        if (hero[i].maxstamina < 10) hero[i].maxstamina = 10;
        hero[i].stamina = hero[i].maxstamina / 2 + 1;
        drain = randGen.nextInt(3);
        hero[i].vitality -= drain;
        if (hero[i].vitality < 1) hero[i].vitality = 1;
        hero[i].setMaxLoad();
        hupdate();
        needredraw = true;
    ...
    public void saveGame() {
        ...
        try {
            ...
            for (Enumeration e = dmmons.elements(); e.hasMoreElements(); ) {
                ...
                if (tempmon.gamewin) {
                    so.writeUTF(tempmon.endanim);
                    so.writeUTF(tempmon.endsound);
                }
    ...
    public boolean loadGame(boolean fromtitle) {
        return loadGame(fromtitle, false);
    }
    
    public boolean loadGame(boolean fromtitle, boolean nocharload) {
        boolean worked = loadGame(fromtitle, nocharload, false);
        while (!worked) {
    ...
    public boolean loadGame(boolean fromtitle, boolean nocharload, boolean junk) {
        try {
            if (!fromtitle) {
                if (!nocharload) {
                    spellsheet.setVisible(false);
            ...
            if (!ver.equals(version)) {
                ...
                if (!this.isShowing())
                    JOptionPane.showMessageDialog(this, "Incorrect Map Version: Found " + ver + ", need " + version, "Error!", JOptionPane.ERROR_MESSAGE);
                else
                    JOptionPane.showMessageDialog(this, "Incorrect Map Version: Found " + ver + ", need " + version + "\nLoad another game or exit and restart.", "Error!", JOptionPane.ERROR_MESSAGE);
                return false;
            }
            //skip over map start info
            si.readBoolean();
        ...
        //catch (InterruptedException e) { System.out.println("Interrupted!"); if (!fromtitle) { if (!sheet) centerlay.show(centerpanel,"dview"); else centerlay.show(centerpanel,"hsheet"); } return true; }
            ...
            //pop up a dialog too
            if (!this.isShowing())
                JOptionPane.showMessageDialog(this, "Unable to load game!", "Error!", JOptionPane.ERROR_MESSAGE);
            else
    ...
    static public MapObject loadMapObject(ObjectInputStream si) throws IOException, ClassNotFoundException {
        return loadMapObject(si, null);
    }
    
    static public MapObject loadMapObject(ObjectInputStream si, MapObject oldmap) throws IOException, ClassNotFoundException {
        char mapchar;
        MapObject m = null;
        //boolean canHoldItems,isPassable,canPassProjs,canPassMons,canPassImmaterial,drawItems,drawFurtherItems,hasParty,hasMons,hasImmaterialMons,hasItems;
    ...
    public void gameOver() {
        ...
        changeCursor();
        stopSounds(true);
        ImageIcon endicon = new ImageIcon(tk.getImage("theend.gif"));
    ...
    class DungView extends JComponent { //Canvas {
        //Image offscreen;
                ...
                switch (facing) {
                    case NORTH:
                        for (int i = 0; i < 4; i++) {
                            ...
                            for (int j = 0; j < 5; j++) {
                                xtest = partyx - 2 + j;
                                if (xtest < 0 || ytest < 0 || xtest > (mapwidth - 1) || ytest > (mapheight - 1))
                                    visibleg[i][j] = outWall;
    ...
    class DungClick extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if (SwingUtilities.isRightMouseButton(e) && numheroes > 0) {
                ...
                showCenter(herosheet);
                return;
            } else if (clickqueue.isEmpty()) clickqueue.add(e.getX() + "," + e.getY());
        }
        ...
        public void processClick(int x, int y) {
            ...
            if (xadjust >= 0 && yadjust >= 0 && xadjust < mapwidth && yadjust < mapheight)
                tocheck = DungeonMap[level][xadjust][yadjust];
            ...
            if (iteminhand) { //drop,throw,use key/coin,push button
                if (y < 236) { //was 190 then 220
                    ...
                    if (doorkeyflag) doorkeyflag = false;
                        ...
                        if (inhand.number != 71 || tocheck.mapchar == ']' || tocheck.mapchar == '[' || tocheck.mapchar == 'f')
                    ...
                    //else if (x<336 && x>114 && (tocheck.canPassProjs || tocheck instanceof Door || tocheck instanceof Stairs)) {//was <350, >100
                        ...
                        if (hero[leader].stamina < hero[leader].maxstamina / 5 || hero[leader].load > hero[leader].maxload)
                            hero[leader].weaponcount += 4; //slower if low stamina or overloaded
                        hero[leader].repaint();
                ...
                if (tocheck.canHoldItems && tocheck.hasItems) {
                    ...
                    if (gridadjust != 0) {
                        ...
                        if (tempmon == null)
                            tempmon = (Monster) dmmons.get(level + "," + xadjust + "," + yadjust + "," + 5);
                        if (tempmon != null && !tempmon.isflying && tempmon.currentai != Monster.FRIENDLYNOMOVE && tempmon.currentai != Monster.FRIENDLYMOVE)
                            return;
                    }
                    ...
                    if (inhand != null) {
                        ...
                        hero[leader].load += inhand.weight;
                        if (inhand.number == 8) {
                            Compass.addCompass(inhand);
                            ((Compass) inhand).setPic(facing);
                        } else if (inhand.number == 5) {
                            //search chest for compasses
                            for (int j = 0; j < 6; j++) {
                                Item citem = ((Chest) inhand).itemAt(j);
                                if (citem != null && citem.number == 8) {
                                    Compass.addCompass(citem);
                                    ((Compass) citem).setPic(facing);
                                }
                            }
                        }
                        changeCursor();
    ...
    class HeroClick extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if (sleeping) {
            ...
            } else if ((y > 17 && y < 83) || SwingUtilities.isRightMouseButton(e)) {
                if (!sheet) {
                ...
                if (iteminhand) {
                    clickedhero.load += inhand.weight;
                    hero[leader].load -= inhand.weight;
                }
            ...
            //weapon hand
                ...
                if (clickedhero.weapon != null) {
                    clickedhero.load -= clickedhero.weapon.weight;
                    hero[leader].load += clickedhero.weapon.weight;
                }
                if (!iteminhand && !clickedhero.weapon.name.equals("Fist/Foot")) {
                    ...
                    if (inhand.type == Item.WEAPON || inhand.type == Item.SHIELD || inhand.number == 4)
                        inhand.unEquipEffect(clickedhero);
                    clickedhero.repaint();
                    if (sheet) herosheet.repaint();
                    weaponsheet.repaint();
                    changeCursor();
                    ...
                    if (!clickedhero.weapon.name.equals("Fist/Foot")) {
                        ...
                        if (inhand.type == Item.WEAPON || inhand.type == Item.SHIELD || inhand.number == 4)
                            inhand.unEquipEffect(clickedhero);
                        if (clickedhero.weapon.type == Item.WEAPON || clickedhero.weapon.type == Item.SHIELD || clickedhero.weapon.number == 4)
                            clickedhero.weapon.equipEffect(clickedhero);
                    } else {
                        clickedhero.weapon = inhand;
                        iteminhand = false;
                        if (clickedhero.weapon.number == 9) {
            ...
            //action hand
                ...
                if (clickedhero.hand != null) {
                    clickedhero.load -= clickedhero.hand.weight;
                    hero[leader].load += clickedhero.hand.weight;
                }
    ...
    class HeroSheet extends JComponent { //Canvas {
        Hero hero;
        Image offscreen;
        ...
        public void paint(Graphics g) {
            ...
            if (hero.weapon == null || skipchestscroll || (hero.weapon.number != 5 && hero.weapon.number != 4)) {
                ...
                while (hero.weapon.scroll[messlength - 1].equals("")) {
                    messlength--;
                }
            ...
            for (int i = 0; i < 8; i++) {
                if (hero.pack[i + 8] != null) {
                    ...
                    if (hero.pack[i + 8].cursed > 0 && hero.pack[i + 8].cursefound)
                        curseg.fillRect(166 + (34 * i), 66, 32, 32);
                }
            }
            for (int i = 0; i < 2; i++) {
                if (hero.quiver[i] != null) {
                    ...
                    if (hero.quiver[i].cursed > 0 && hero.quiver[i].cursefound)
                        curseg.fillRect(124 + (34 * i), 167, 32, 32);
                }
            }
            for (int i = 0; i < 2; i++) {
                if (hero.quiver[i + 2] != null) {
                    offg.drawImage(hero.quiver[i + 2].pic, 124 + (34 * i), 201, this);
                    if (hero.quiver[i + 2].cursed > 0 && hero.quiver[i + 2].cursefound)
                        curseg.fillRect(124 + (34 * i), 201, 32, 32);
                }
            }
            for (int i = 0; i < 2; i++) {
                if (hero.quiver[i + 4] != null) {
                    offg.drawImage(hero.quiver[i + 4].pic, 124 + (34 * i), 235, this);
                    if (hero.quiver[i + 4].cursed > 0 && hero.quiver[i + 4].cursefound)
                        curseg.fillRect(124 + (34 * i), 235, 32, 32);
                }
            }
            g.drawImage(offscreen, 0, 0, null);
        }
        ...
        public void showItem() {
            if (inhand.number == 4) {
                ...
                while (inhand.scroll[messlength - 1].equals("")) {
                    messlength--;
                }
                int startdraw = 188 - messlength * 10;
                offg.setColor(new Color(30, 30, 30));
    ...
    class SheetClick extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            } else if (x > 420 && x < 440 && y < 25 && y > 5 || SwingUtilities.isRightMouseButton(e)) {
                sheet = false;
                showCenter(dview);
                hupdate();
            }
            ...
            //weapon hand
                ...
                if (herosheet.hero.weapon != null) {
                    ...
                    hero[leader].load += herosheet.hero.weapon.weight;
                }
                if (!iteminhand && !herosheet.hero.weapon.name.equals("Fist/Foot")) {
                    ...
                    if (!herosheet.hero.weapon.name.equals("Fist/Foot")) {
                        ...
                        if (inhand.type == Item.WEAPON || inhand.type == Item.SHIELD || inhand.number == 4)
                            inhand.unEquipEffect(herosheet.hero);
                        if (herosheet.hero.weapon.type == Item.WEAPON || herosheet.hero.weapon.type == Item.SHIELD || herosheet.hero.weapon.number == 4)
                            herosheet.hero.weapon.equipEffect(herosheet.hero);
                    } else {
                        herosheet.hero.weapon = inhand;
                        iteminhand = false;
                        if (herosheet.hero.weapon.number == 9) {
            ...
            //action hand
                ...
                if (herosheet.hero.hand != null) {
                    ...
                    hero[leader].load += herosheet.hero.hand.weight;
                }
            ...
            //pack
                ...
                if (herosheet.hero.pack[(x - 166) / 34 + i] != null) {
                    ...
                    hero[leader].load += herosheet.hero.pack[(x - 166) / 34 + i].weight;
                }
            ...
            //else if (x>227 && x<431 && y>206 && y<239 && herosheet.hero.weapon!=null && herosheet.hero.weapon.number==5) {
                ...
                if (!iteminhand && ((Chest) herosheet.hero.weapon).contents[(x - 227) / 34 + ymod] != null) {
                    herosheet.hero.load -= ((Chest) herosheet.hero.weapon).contents[(x - 227) / 34 + ymod].weight;
                    hero[leader].load += ((Chest) herosheet.hero.weapon).contents[(x - 227) / 34 + ymod].weight;
                    iteminhand = true;
    ...
    class MirrorClick extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            x = e.getX();
            y = e.getY();
            if ((x > 420 && x < 440 && y < 25 && y > 5) || (x > 205 && x < 439 && y > 223 && y < 250) || SwingUtilities.isRightMouseButton(e)) {
                sheet = false;
    ...
    class DungMove extends KeyAdapter {
        ...
        public void partyTurn(boolean left) {
            ...
            for (Iterator i = dmprojs.iterator(); i.hasNext(); ) {
                ...
                if (tempp.it != null && tempp.it.hasthrowpic) {
                    int s = 2; //left
                    if (facing == tempp.direction) {
                        s = 0;
                    } //away
                    else if ((facing - tempp.direction) % 2 == 0) {
                        s = 1;
                    }//towards - was Math.abs(facing-tem...)%2
                    else if (facing == (tempp.direction + 1) % 4) {
                        s = 3;
                    } //right = dpic
            ...
            } else if (DungeonMap[level][partyx][partyy] instanceof MultFloorSwitch2)
                ((MultFloorSwitch2) DungeonMap[level][partyx][partyy]).turnTest(oldfacing);
            if (mirrorback != oldmirror && !nomirroradjust && (oldx != partyx || oldy != partyy || oldlevel != level || newfacing != facing))
                mirrorback = !mirrorback;
            nomirroradjust = false;
        }
        ...
        public void partyMove(int dir) {
            ...
            if (DungeonMap[level][newx][newy].isPassable && !DungeonMap[level][newx][newy].hasMons) {
                ...
                if (!gameover && DungeonMap[level][newx][newy].hasParty) {
                    ...
                    if (numheroes != 0 && newsquare == DungeonMap[level][newx][newy])
                        DungeonMap[level][newx][newy].tryFloorSwitch(MapObject.PARTYSTEPPINGON);
                    ...
                    if (!gameover) {
                        for (int i = 0; i < numheroes; i++) {
                            ...
                            if (hero[i].load > hero[i].maxload)
                                hero[i].walkcounter++; //if overloaded, stamina drains faster
        ...
        class WeaponSheet extends ImageTilePanel implements ActionListener{
                ...
                public void functionsEnable(boolean enab) {
    ...
    class Formation extends JPanel implements MouseListener {
        ...
        public Formation() {
            ...
            for (int i = 0; i < 4; i++) {
                heroicons[i] = new ImageIcon("heroicon" + i + ".gif");
    ...
    class SpellSheet extends JPanel implements MouseListener, MouseMotionListener {
        ...
        public SpellSheet() {
            ...
            setMaximumSize(new Dimension(160, 80));
            setCursor(handcursor);
            backpic = tk.createImage("Icons" + File.separator + "spellsheet.gif");
            Image src = tk.createImage("spell.gif");
            ImageTracker.addImage(backpic, 3);
            ImageTracker.addImage(src, 3);
            try {
                ImageTracker.waitForID(3, 5000);
            } catch (InterruptedException e) {
            }
        ...
        public void paintComponent(Graphics g) {
            ...
            //draw spell symbols
            int index = hero[spellready].currentspell.length();
            if (index == 4) index = 0;
            for (int i = 0; i < 6; i++) {
        ...
        public void mouseReleased(MouseEvent e) {
            if (pressed == 0) return;
                ...
                if (hero[spellready].currentspell.length() > 0)
                    mananeed = SYMBOLCOST[(pressed - 1) + 6 * (hero[spellready].currentspell.length() - 1)][Integer.parseInt(hero[spellready].currentspell.substring(0, 1)) - 1];
                if (hero[spellready].mana >= mananeed) {
                    hero[spellready].mana -= mananeed;
            ...
            pressed = 0;
            over = -1;
            mouseMoved(new MouseEvent(this, MouseEvent.MOUSE_MOVED, 0, 0, e.getX(), e.getY(), 0, false));
    ...
    class WeaponSheet extends JPanel implements MouseListener {
        ...
        final Color presscolor = new Color(115, 115, 200);
        final Color disabcolor = new Color(125, 125, 145);
        final Color darkcolor = new Color(55, 55, 75);
        final Color greycolor = new Color(80, 80, 80);
        final Color hitcolor = new Color(240, 170, 115);
        int pressed = 0, abilitymod = 0;
        boolean[] showing = new boolean[3];
        String hittext;
        boolean showingspecials = false;
        ...
        public WeaponSheet() {
            ...
            setCursor(handcursor);
            addMouseListener(this);
            backpic = tk.createImage("Icons" + File.separator + "weaponsheet.gif");
            hit1 = tk.createImage("Icons" + File.separator + "wephit.gif");
            hit2 = tk.createImage("Icons" + File.separator + "wephit2.gif");
            hit3 = tk.createImage("Icons" + File.separator + "wephit3.gif");
            miss = tk.createImage("Icons" + File.separator + "wepmiss.gif");
            uparrow = tk.createImage("uparrow.gif");
            downarrow = tk.createImage("downarrow.gif");
            ImageTracker.addImage(backpic, 3);
        ...
        public void updateSpecials() {
            for (int j = 0; j < numheroes; j++) {
                if (hero[j].abilities != null) for (int i = 0; i < hero[j].abilities.length; i++) {
                    ...
                    if (hero[j].abilities[i].count > 0) hero[j].abilities[i].count--;
                    //if current hero, update button if necessary
                    if (showingspecials && weaponready == j) {
                        setAbilities();
                        repaint();
                    }
                }
        ...
        private void setAbilities() {
            ...
            if (abilities[2] == null && abilitymod > 0) {
                abilitymod--;
                setAbilities();
            }
        }
        ...
        public void mousePressed(MouseEvent e) {
            ...
            //switch from special abilities to weapon stuff
                ...
                //do ability buttons
                    ...
                    int f = (y - 34) / 20;
                    if (abilities[f] != null && abilities[f].count <= 0 && abilities[f].mana <= hero[weaponready].mana)
                        pressed = f + 4;
                }
            }
            repaint();
        ...
        public void mouseReleased(MouseEvent e) {
            if (pressed == 0) return;
                ...
                if (!hero[weaponready].doAbility(abilities[i].getActionName(), abilities[i].power, abilities[i].classgain, abilities[i].data)) {
                } //do nothing if action name not found
                else {
                    //play sound if not null
                    if (abilities[i].sound != null && !abilities[i].sound.equals(""))
                        playSound(abilities[i].sound, -1, -1, 0);
                    //decrease mana if supposed to
    ...
    class ArrowSheet extends JPanel implements MouseListener {
        ...
        public ArrowSheet() {
            ...
            setCursor(handcursor);
            addMouseListener(this);
            unpressed = tk.createImage("Icons" + File.separator + "movement.gif");
            moveforward = tk.createImage("Icons" + File.separator + "forward.gif");
            moveback = tk.createImage("Icons" + File.separator + "back.gif");
            moveleft = tk.createImage("Icons" + File.separator + "left.gif");
            moveright = tk.createImage("Icons" + File.separator + "right.gif");
            turnleft = tk.createImage("Icons" + File.separator + "turnleft.gif");
            turnright = tk.createImage("Icons" + File.separator + "turnright.gif");
    ...
    class Message extends JComponent {//Canvas {
    ...
    class Holding extends JComponent {//Canvas {
        //Image offscreen;
        //Graphics offg;
                ...
                do {
                    ...
                    if (direction == NORTH || direction == SOUTH) {
                        ...
                        if (randGen.nextBoolean()) {
                            if (dirchange.contains("1") && x > 0 && !(DungeonMap[level][x - 1][y] instanceof Wall) && DungeonMap[level][x - 1][y].mapchar != '2') {
                                dir = 1;
                                dirchange.remove("1");
                            } else if (dirchange.contains("3")) {
                                dir = 3;
                                dirchange.remove("3");
                            } else {
                                dir = 2 - direction;
                            ...
                            if (dirchange.contains("3") && x < mapwidth - 1 && !(DungeonMap[level][x + 1][y] instanceof Wall) && DungeonMap[level][x + 1][y].mapchar != '2') {
                                dir = 3;
                                dirchange.remove("3");
                            } else if (dirchange.contains("1")) {
                                dir = 1;
                                dirchange.remove("1");
                            } else {
                                dir = 2 - direction;
                        ...
                        if (randGen.nextBoolean()) {
                            if (dirchange.contains("0") && y > 0 && !(DungeonMap[level][x][y - 1] instanceof Wall) && DungeonMap[level][x][y - 1].mapchar != '2') {
                                dir = 0;
                                dirchange.remove("0");
                            } else if (dirchange.contains("2")) {
                                dir = 2;
                                dirchange.remove("2");
                            } else {
                                dir = (direction + 2) % 4;
                            ...
                            if (dirchange.contains("2") && y < mapheight - 1 && !(DungeonMap[level][x][y + 1] instanceof Wall) && DungeonMap[level][x][y + 1].mapchar != '2') {
                                dir = 2;
                                dirchange.remove("2");
                            } else if (dirchange.contains("0")) {
                                dir = 0;
                                dirchange.remove("0");
                            } else {
                                dir = (direction + 2) % 4;
            ...
            if (it != null) { //item proj, not spell
                ...
                if (!justthrown && DungeonMap[level][x][y].hasParty) {
                    ...
                    if (hit != -1) {
                        ...
                        if (it.poisonous > 0 && randGen.nextBoolean()) {
                            hero[hit].poison += it.poisonous;
                ...
                //hit mons?
                    ...
                    if (tempmon != null) {
                        if ((hitsImmaterial || !tempmon.isImmaterial) && (tempmon.hurtitem == 0 || it.number == 215 || it.number == tempmon.hurtitem)) {
                            ...
                            if (!tempmon.isImmaterial && it.poisonous > 0 && randGen.nextBoolean()) {
                                tempmon.poisonpow += it.poisonous;
                                tempmon.ispoisoned = true;
                            }
                            //chance of sticking in mon (not heavily armored ones like stone golems, though)
                            if (!tempmon.isImmaterial && !tempmon.isdying && tempmon.defense < 80 && it.projtype > 0 && it.number != 266 && randGen.nextInt(10) == 0) {
                                it.shotpow = 0;
                                tempmon.carrying.add(it);
                                shoulddrop = false;
                            }
                        }
                    }
                ...
                isending = true;
                if (level == dmnew.level) {
                    if (sp.number == 44 || sp.number == 46 || sp.number == 335 || sp.number == 642 || sp.number == 365)
                        playSound("fball.wav", x, y);
                ...
                if (!hitsImmaterial && DungeonMap[level][x][y].hasParty) {
                    if (sp.ismultiple) {
                        if (sp.number == 362) {
                            slowcount += sp.power * 3;
                            message.setMessage("Party Slowed.", 4);
                        }
                        for (int i = 0; i < numheroes; i++) {
                            if (sp.number == 362) {
                                //slow
                                if (hero[i].dexterityboost > -sp.power) {
                                    ...
                                    if (hero[i].dexterity + hero[i].dexterityboost <= 0)
                                        hero[i].dexterityboost = 1 - hero[i].dexterity;
                                    hero[i].dexterity += hero[i].dexterityboost;
                                }
                        ...
                        if (hit != -1) {
                            if (sp.number == 51) {
                                ...
                                if (hero[hit].strengthboost > -sp.power) {
                                    ...
                                    if (hero[hit].strength + hero[hit].strengthboost <= 0)
                                        hero[hit].strengthboost = 1 - hero[hit].strength;
                                    hero[hit].strength += hero[hit].strengthboost;
                                    hero[hit].setMaxLoad();
                                }
                                if (hero[hit].vitalityboost > -sp.power) {
                                    ...
                                    if (hero[hit].vitality + hero[hit].vitalityboost <= 0)
                                        hero[hit].vitalityboost = 1 - hero[hit].vitality;
                                    hero[hit].vitality += hero[hit].vitalityboost;
                                }
                                ...
                                if (hero[hit].intelligenceboost > -sp.power) {
                                    ...
                                    if (hero[hit].intelligenceboost < -sp.power)
                                        hero[hit].intelligenceboost = -sp.power;
                                    if (hero[hit].intelligence + hero[hit].intelligenceboost <= 0)
                                        hero[hit].intelligenceboost = 1 - hero[hit].intelligence;
                                    hero[hit].intelligence += hero[hit].intelligenceboost;
                                }
                                if (hero[hit].wisdomboost > -sp.power) {
                                    ...
                                    if (hero[hit].wisdom + hero[hit].wisdomboost <= 0)
                                        hero[hit].wisdomboost = 1 - hero[hit].wisdom;
                                    hero[hit].wisdom += hero[hit].wisdomboost;
                                }
                                hero[hit].damage(1, SPELLHIT);
                ...
                //hit mon?
                    ...
                    if (sp.ismultiple) {
                            ...
                            if (tempmon != null && (hitsImmaterial == tempmon.isImmaterial)) {
                                if (sp.number == 362) {
                                    ...
                                    if (tempmon.speedboost > -sp.power) {
                                        ...
                                        if (tempmon.speed + tempmon.speedboost <= 0)
                                            tempmon.speedboost = 1 - tempmon.speed;
                                        tempmon.speed += tempmon.speedboost;
                                    }
                        ...
                        if (tempmon != null && (hitsImmaterial == tempmon.isImmaterial || (sp.number == 0 && tempmon.number == 26))) {
                            ...
                            if (sp.number == 51 && !tempmon.isImmaterial && tempmon.number != 26) {
                                tempmon.damage(pow, SPELLHIT);
                                tempmon.poisonpow += (sp.gain * 2 + 1);
                                tempmon.ispoisoned = true;
                                ...
                                if (DungeonMap[level][tempmon.x][tempmon.y].mapchar != 'F' || ((FDecoration) DungeonMap[level][tempmon.x][tempmon.y]).number != 2)
                                    surrounded = false;
                                else if (fluxcages.get(level + "," + tempmon.x + "," + tempmon.y) == null)
                                    surrounded = false;
                                    /*party does count*/
                                else if ((!(DungeonMap[level][tempmon.x][tempmon.y - 1] instanceof Wall) || DungeonMap[level][tempmon.x][tempmon.y - 1].mapchar == '2') && (partyx != tempmon.x || partyy != tempmon.y - 1) && fluxcages.get(level + "," + tempmon.x + "," + (tempmon.y - 1)) == null)
                                    surrounded = false;
                                else if ((!(DungeonMap[level][tempmon.x][tempmon.y + 1] instanceof Wall) || DungeonMap[level][tempmon.x][tempmon.y + 1].mapchar == '2') && (partyx != tempmon.x || partyy != tempmon.y + 1) && fluxcages.get(level + "," + tempmon.x + "," + (tempmon.y + 1)) == null)
                                    surrounded = false;
                                else if ((!(DungeonMap[level][tempmon.x - 1][tempmon.y] instanceof Wall) || DungeonMap[level][tempmon.x - 1][tempmon.y].mapchar == '2') && (partyx != tempmon.x - 1 || partyy != tempmon.y) && fluxcages.get(level + "," + (tempmon.x - 1) + "," + tempmon.y) == null)
                                    surrounded = false;
                                else if ((!(DungeonMap[level][tempmon.x + 1][tempmon.y] instanceof Wall) || DungeonMap[level][tempmon.x + 1][tempmon.y].mapchar == '2') && (partyx != tempmon.x + 1 || partyy != tempmon.y) && fluxcages.get(level + "," + (tempmon.x + 1) + "," + tempmon.y) == null)
                                    surrounded = false;
                                                /*party doesn't count
                                ...
                                if (tempmon.manapowerboost > -sp.power) {
                                    ...
                                    if (tempmon.manapower + tempmon.manapowerboost <= 0)
                                        tempmon.manapowerboost = 1 - tempmon.manapower;
                                    tempmon.manapower += tempmon.manapowerboost;
                                }
                            } else if (sp.number == 664) {
                                //strip defenses
                ...
                if (sp.number == 31 || sp.number == 61) {
                    PoisonCloud cloud = new PoisonCloud(level, x, y, sp.gain);
                }
                if (level == dmnew.level) {
                    int xdist = partyx - x;
                    if (xdist < 0) xdist *= -1;
    ...
    class Monster {
        ...
        public void timePass() {
            ...
            breakdoor = false;
            if (freezelife > 0 && number != 26 && number != 27)
                return; //if all life frozen, doesn't affect chaos or demon lord
            if (timecounter < 0) {
                timecounter++;
                timecounter += sleeper;
                return;
            ...
            if (movecounter > movespeed) {
                ...
                if (number == 26 && fluxchanging && fluxcages.get(level + "," + x + "," + y) != null) {
                    runcounter = 1;
                    currentai = RUN;
                }
                doAI();
        ...
        public void hitHero() {
            ...
            if (steal != 0 && (steal == 4 || (steal == 2 && randGen.nextBoolean()) || (steal == 1 && randGen.nextInt(4) == 0) || (steal == 3 && randGen.nextInt(4) != 0))) {
                ...
                if (!hero[who].weapon.name.equals("Fist/Foot") && hero[who].weapon.number != 219 && hero[who].weapon.number != 261) {
                    ...
                    if (hero[who].weapon.type == Item.WEAPON || hero[who].weapon.type == Item.SHIELD)
                        hero[who].weapon.unEquipEffect(hero[who]);
                    ...
                    tryval.add(new Integer(0));
                    tryval.add(new Integer(1));
                    tryval.add(new Integer(2));
                    tryval.add(new Integer(3));
                    tryval.add(new Integer(4));
                    tryval.add(new Integer(5));
                    tryval.add(new Integer(6));
                    tryval.add(new Integer(7));
                    tryval.add(new Integer(8));
                ...
                if (HITANDRUN && (!found || randGen.nextInt(32) != 0)) {
                    runcounter = 40 + randGen.nextInt(10);
                    currentai = RUN;
                }
                return;
            }
            if (!sleeping && randGen.nextInt(20) == 0) {
                movecounter -= 4;
                message.setMessage(name + ": Critical Miss", 5);
                return;
            }
            ...
            if (hero[who].falseimage > 0 && (didhit || randGen.nextBoolean())) {
                didhit = false;
                hero[who].falseimage--;
                if (hero[who].falseimage > 0)
                    message.setMessage("A false image around " + hero[who].name + " is destroyed.", hero[who].heronumber);
                else
                    message.setMessage("The last false image around " + hero[who].name + " is destroyed.", hero[who].heronumber);
            }
            ...
            if (didhit) {
                ...
                if (speed > 50 && (randGen.nextInt(20) == 0 || (sleeping && randGen.nextInt(5) == 0))) {
                    pow = 3 * pow / 2;
                    message.setMessage(name + ": Critical Hit", 5);
                }
                if (poison > 0 && (randGen.nextInt(10) > 4 || (DIFFICULTY > 0 && randGen.nextInt(10 + DIFFICULTY * 5) > 4))) {
                    hero[who].poison += poison;
                    hero[who].ispoisoned = true;
                ...
                hupdate();
                if (HITANDRUN) {
                    runcounter = 12 + (Math.abs(randGen.nextInt()) % 8);
                    currentai = RUN;
                }
            }
        }
            ...
            while (movecounter > movespeed) { //go thru until move or attack
                ...
                switch (currentai) {
                    ...
                    case RANDOM:
                        ...
                        } else if (level == dmnew.level && xdist < 5 && ydist < 5)
                            newai = defaultai; //if near party (within 5 squares)
                        
                        //chaos and sorcerer may teleport
                        if ((number == 26 || (canteleport && (!silenced || randGen.nextBoolean()))) && randGen.nextInt(20) == 0 && teleport()) {
                            moveattack = moncycle;
                        } else {
                    ...
                    case GOTOWARDS:
                        ...
                        if (health < maxhealth / 5 && hasheal && !silenced && mana > 59)
                            useHealMagic(); //power depends on how powerful a mage mon is
                        ...
                        if (canattack)
                            doAttack(); //do attack or use close-range magic - have chance of setting hurt (even though not hurt) and entering run - dodge
                            ...
                            //get random boolean to determine whether to start checking x or y values
                                ...
                                if (x < partyx && canMove(EAST)) monMove(EAST);
                                    ...
                                    randomcounter = 3;
                                    newai = RANDOM;
                                } //couldn't move towards - obstacle - try get around by moving a few random steps - this doesn't work that great...
                    ...
                    case RUN: //move away from party (can be fixed number of steps - giggler, horn of fear?)
                        ...
                        if (((canteleport && (!silenced || randGen.nextBoolean())) || (number == 26 && wasstuck)) && teleport()) {
                            wasstuck = false;
                            moveattack = moncycle;
                        } else if (wasstuck && canattack) {
                            wasstuck = false;
                            doAttack();
                        } else if (canattack && !wasfrightened && randGen.nextBoolean() && (!HITANDRUN || randGen.nextInt(6) == 0)) {
                            waitattack = true;
                            doAttack();
                        }//chance of turning and attacking in one move
                        else if (randGen.nextBoolean()) { //check x values first, y's if can't move in x
                            if (x == partyx && randGen.nextBoolean()) {
                                if (randGen.nextBoolean() && canMove(EAST)) monMove(EAST);
                                    ...
                                    if ((!wasstuck || olddir == -1 || olddir != NORTH) && canMove(SOUTH))
                                        monMove(SOUTH);
                                } else if (y < partyy) {
                                    if ((!wasstuck || olddir == -1 || olddir != SOUTH) && canMove(NORTH))
                                        monMove(NORTH);
                                } else {
                                    wasstuck = true;
                                    randomcounter = 3;
                                ...
                                if ((!wasstuck || olddir == -1 || olddir != WEST) && canMove(EAST)) monMove(EAST);
                            } else if (x < partyx) {
                                if ((!wasstuck || olddir == -1 || olddir != EAST) && canMove(WEST)) monMove(WEST);
                            } else if (y > partyy) {
                                if ((!wasstuck || olddir == -1 || olddir != NORTH) && canMove(SOUTH)) monMove(SOUTH);
                            } else if (y < partyy) {
                                if ((!wasstuck || olddir == -1 || olddir != SOUTH) && canMove(NORTH)) monMove(NORTH);
                            }
                            ...
                            if (movecounter > movespeed) {
                                ...
                                if (count < 4 && randGen.nextInt(15) > 0) {
                                    olddir = dir;
                                    monMove(dir);
                                }
                                ...
                                //}
                                    ...
                                    if (randGen.nextBoolean()) {
                                        olddir = -1;
                                        newai = GOTOWARDS;
                                    } else if (x > partyx && canMove(WEST)) {
                                        wasstuck = true;
                                        olddir = WEST;
                                        monMove(WEST);
                                    } else if (x < partyx && canMove(EAST)) {
                                        wasstuck = true;
                                        olddir = EAST;
                                        monMove(EAST);
                                    } else if (y > partyy && canMove(NORTH)) {
                                        wasstuck = true;
                                        olddir = NORTH;
                                        monMove(NORTH);
                                    } else if (y < partyy && canMove(SOUTH)) {
                                        wasstuck = true;
                                        olddir = SOUTH;
                                        monMove(SOUTH);
                                    } else movecounter = 0;
                        ...
                        if (hurt && hasheal && !silenced && mana > 59 && randGen.nextBoolean()) {
                            useHealMagic();
                        } else if (hurt && health > .2 * maxhealth) {
                            hurt = false;
                            if (runcounter == 0) {
                                olddir = -1;
                                newai = defaultai;
                            }
                        }//no longer hurting, perhaps no longer running
                        if (!hurt && runcounter > 0) {
                            runcounter--;
                            if (runcounter == 0) {
                                olddir = -1;
                                newai = defaultai;
                    ...
                    case STAYBACK: //wizards, archers?
                        ...
                        if (ammo == 0 && (!hasmagic || mana < minproj || silenced)) {
                            if (randGen.nextBoolean()) newai = GOTOWARDS;
                            else {
                                runcounter = 5;
                                newai = RUN;
                            }
                            ...
                            if ((ydist > 1 || (!canattack && randGen.nextInt(3) != 0)) && randGen.nextInt() % 10 < 6) {
                                ...
                                else if ((subsquare == 0 || subsquare == 3) && canMove(EAST)) {
                                    monMove(EAST);
                                    movecounter = attackspeed;
                                } else if ((subsquare == 1 || subsquare == 2) && canMove(WEST)) {
                                    monMove(WEST);
                                    movecounter = attackspeed;
                                } else {
                                    randomcounter = 3;
                            ...
                            if ((xdist > 1 || (!canattack && randGen.nextInt(3) != 0)) && randGen.nextInt() % 10 < 6) {
                                ...
                                else if ((subsquare == 0 || subsquare == 1) && canMove(SOUTH)) {
                                    monMove(SOUTH);
                                    movecounter = attackspeed;
                                } else if ((subsquare == 2 || subsquare == 3) && canMove(NORTH)) {
                                    monMove(NORTH);
                                    movecounter = attackspeed;
                                } else {
                                    randomcounter = 3;
                    ...
                    default:
                        ...
                        if (hasheal && !silenced && (health < .2 * maxhealth) && mana > 59)
                            useHealMagic();//heal self if hurting (if can)
                        ...
                        if (level == dmnew.level && xdist < 5 && ydist < 5) {
                            ...
                            if (useammo && ammo == 0) {
                                defaultai = GOTOWARDS;
                                newai = GOTOWARDS;
                            } //run out of ammo turns guard into gotoward
                        }
                        break;
                ...
                if (currentai != GUARD && currentai != FRIENDLYMOVE && xdist > 4 && ydist > 4 && (currentai != RANDOM || randomcounter > 0)) {
                    randomcounter = 0;
                    currentai = RANDOM;
                }
            } //end while
        ...
        public boolean canMove(int d) {
            ...
            if ((!DungeonMap[level][xadjust][yadjust].isPassable && !isImmaterial) || DungeonMap[level][xadjust][yadjust].hasParty || DungeonMap[level][xadjust][yadjust].numProjs > 0 || DungeonMap[level][x][y].numProjs > 0)
                movetest = false;
            else if (DungeonMap[level][xadjust][yadjust].hasMons && (newsub == 5 || dmmons.get(level + "," + xadjust + "," + yadjust + "," + newsub) != null || dmmons.get(level + "," + xadjust + "," + yadjust + "," + 5) != null))
                movetest = false;
            ...
            else if (!isImmaterial && !ignoremons && DungeonMap[level][xadjust][yadjust].hasCloud && randGen.nextBoolean())
                movetest = false;
            ...
            else if (!isflying && (DungeonMap[level][xadjust][yadjust] instanceof FulYaPit || (DungeonMap[level][xadjust][yadjust] instanceof Pit && (((Pit) DungeonMap[level][xadjust][yadjust]).isOpen || (!ignoremons && ((Pit) DungeonMap[level][xadjust][yadjust]).isActive)))))
                movetest = false;//don't step into open pit (unless can fly)
            else if (!canusestairs && DungeonMap[level][xadjust][yadjust] instanceof Stairs) movetest = false;
                //chaos and fluxcages:
            else if (number == 26 && fluxchanging && fluxcages.get(level + "," + xadjust + "," + yadjust) != null)
                movetest = false;
            ...
            if (!movetest && DungeonMap[level][xadjust][yadjust] instanceof Door && !((Door) DungeonMap[level][xadjust][yadjust]).isOpen) {
                if (!ignoremons && ((Door) DungeonMap[level][xadjust][yadjust]).opentype == Door.BUTTON) {
                    ...
                    if (!DungeonMap[level][xadjust][yadjust].hasMons && !DungeonMap[level][xadjust][yadjust].hasParty && DungeonMap[level][xadjust][yadjust].numProjs == 0 && DungeonMap[level][x][y].numProjs == 0)
                        movetest = true;
                }
                //or, try to break it down (only some mons) if isBreakable and is wooden (pictype==0)
                    ...
                    if (((Door) DungeonMap[level][xadjust][yadjust]).isBroken && !DungeonMap[level][xadjust][yadjust].hasMons && !DungeonMap[level][xadjust][yadjust].hasParty && DungeonMap[level][xadjust][yadjust].numProjs == 0 && DungeonMap[level][x][y].numProjs == 0)
                        movetest = true;
                }
            }
            return movetest;
        ...
        public void monMove(int d) {
            ...
            if (!isImmaterial && DungeonMap[level][xadjust][yadjust].mapchar == 'd' && !DungeonMap[level][xadjust][yadjust].isPassable)
                return;
            
            int oldlevel = level, oldx = x;
            int oldy = y, oldsub = subsquare, oldface = facing;
            ...
            if (!isdying && x == xadjust && y == yadjust && level == oldlevel) {
                ...
                if (!isdying && pickup != 0 && DungeonMap[level][x][y].hasItems && (pickup == 4 || (pickup == 2 && randGen.nextBoolean()) || (pickup == 1 && randGen.nextInt(4) == 0) || (pickup == 3 && randGen.nextInt(4) != 0))) {
                    ...
                    if (tempitem != null) {
                        ...
                        if (useammo && ((tempitem.number > 220 && (tempitem.hasthrowpic || tempitem.number == 266)) || (tempitem.projtype > 0 && tempitem.weight <= 1.0)))
                            ammo++;
                ...
                if (level == dmnew.level && xdist < 6 && ydist < 6) {
                    needredraw = true;
                    if (attackspeed > 0 && ((xdist == 0 && ydist == 1) || (ydist == 0 && xdist == 1)))
                        movecounter = attackspeed;
                    if (!sleeping && !footstep.equals(""))
                        playFootStep(footstep, x, y);//footstep sound (not played when sleeping -> sounds weird because so fast)
                }
                if (movespeed - movecounter < 2)
                    moveattack = moncycle;//prevents double updates (since removed from dmmons and added again while iterating through it)
            }
        }
        ...
        public void doAttack() {
            ...
            if (partyx > x && facing != EAST) {
                facing = EAST;
                noattack = true;
            } else if (partyx < x && facing != WEST) {
                facing = WEST;
                noattack = true;
            } else if (partyy > y && facing != SOUTH) {
                facing = SOUTH;
                noattack = true;
            } else if (partyy < y && facing != NORTH) {
                facing = NORTH;
                noattack = true;
            } else if (alldead) noattack = true;
            if (waitattack && noattack && !alldead) {
                noattack = false;
                waitattack = false;
            }
            ...
            if (!noattack) {
                ...
                if (hasmagic && !silenced && mana >= minproj && randGen.nextInt() % 10 > 0) {
                    if (x < partyx && canDoProj(EAST)) {
                        doProjAttack(EAST);
                        return;
                    } else if (x > partyx && canDoProj(WEST)) {
                        doProjAttack(WEST);
                        return;
                    } else if (y > partyy && canDoProj(NORTH)) {
                        doProjAttack(NORTH);
                        return;
                    } else if (y < partyy && canDoProj(SOUTH)) {
                        doProjAttack(SOUTH);
                        return;
                    }
                    ...
                    try {
                        ...
                        if (hit > 0) {
                            heal(hit);
                            message.setMessage(name + " drains " + hit + " life from " + hero[who].name, 5);
        ...
        public boolean canDoProj(int d) {
            ...
            switch (d) {
                ...
                case NORTH:
                    ...
                    if (heroatsub[(tempsub1 + dmnew.facing) % 4] == -1 && heroatsub[(tempsub2 + dmnew.facing) % 4] == -1) {
                        cancast = false;
                        hashero1 = false;
                    }
                    if (subsquare == 5) {
                        hashero2 = (heroatsub[(1 + dmnew.facing) % 4] != -1 || heroatsub[(2 + dmnew.facing) % 4] != -1);
                        if (hashero2 && (!cancast || randGen.nextBoolean())) {
                            tempsub1 = 1;
                            tempsub2 = 2;
                            cancast = true;
                            castsub = 1;
                    ...
                    while (cancast && testval > partyy) {
                        if (!DungeonMap[level][x][testval].canPassProjs) cancast = false;
                            ...
                            if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != y || 5 != subsquare))
                                cancast = false;
                            else {
                                tempmon = (Monster) dmmons.get(level + "," + x + "," + testval + "," + tempsub1);
                                if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != y || tempsub1 != subsquare))
                                    cancast = false;
                                else {
                                    tempmon = (Monster) dmmons.get(level + "," + x + "," + testval + "," + tempsub2);
                                    if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != y || tempsub2 != subsquare))
                                        cancast = false;
                                }
                            }
                ...
                case EAST:
                    ...
                    while (cancast && testval < partyx) {
                        if (!DungeonMap[level][testval][y].canPassProjs) cancast = false;
                            ...
                            if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != x || 5 != subsquare))
                                cancast = false;
                            else {
                                tempmon = (Monster) dmmons.get(level + "," + testval + "," + y + "," + tempsub1);
                                if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != x || tempsub1 != subsquare))
                                    cancast = false;
                                else {
                                    tempmon = (Monster) dmmons.get(level + "," + testval + "," + y + "," + tempsub2);
                                    if (tempmon != null && !tempmon.isdying && !tempmon.isImmaterial && (testval != x || tempsub2 != subsquare))
                                        cancast = false;
                                }
                            }
            ...
            if (hasmagic && !silenced && mana >= minproj && (ammo == 0 || randGen.nextBoolean())) {
                ...
                if (wep.isbomb) {
                    try {
                        ...
                        p = new Projectile(wep, x, y, dist, dir, projsub);
                    }
        ...
        public int damage(int hit, int type) {
            ...
            if (needitem != 0 && type != STORMHIT && type != FUSEHIT) {
                ...
                if (needhandneck == 0) {
                    ...
                    for (int i = 0; i < numheroes; i++) {
                        if (hero[i].weapon.number == needitem || (hero[i].hand != null && hero[i].hand.number == needitem))
                            found = true;
                            //firestaff+ works for firestaff (but not other way around)
                        else if (needitem == 248 && hero[i].weapon.number == 249 || (hero[i].hand != null && hero[i].hand.number == 249))
                            found = true;
                ...
                if (!found) {
                    if (type != POISONHIT && type != DOORHIT && level == dmnew.level)
                        message.setMessage(name + " is unaffected.", 4);
                    return 0;
                }
            }
            ...
            if (type == DRAINHIT && (isImmaterial || number == 8 || number == 21)) {
                ...
                ydist = y - partyy;
                if (ydist < 0) ydist *= -1;
                if (level == dmnew.level && xdist < 5 && ydist < 5 && type != POISONHIT && type != DOORHIT && type != FUSEHIT)
                    message.setMessage(name + " takes " + hit + " damage.", 5);
            }
            //if (xdist<5 && ydist<5) {
            ...
            if (health < 1) {
                ...
                if (level == dmnew.level && xdist < 5 && ydist < 5) needredraw = true;
                    ...
                    dmmons.remove(level + "," + x + "," + y + "," + subsquare);
                    boolean montest = false;
                    if (subsquare != 5) for (int sub = 0; sub < 3; sub++)
                        if (dmmons.get(level + "," + x + "," + y + "," + sub) != null) montest = true;
                    DungeonMap[level][x][y].hasMons = montest;
                }
    ...
    class Hero extends JComponent {//Canvas {
        String name;
        String lastname;
        String picname;
        Image pic;
                ...
                if (timecounter > 250) {
                    ...
                    if (!sleeping && weapon.number == 215) {
                        ...
                        if (!sleeping && numheroes > 1 && randGen.nextInt(60) == 0) {
                            ...
                            while (hero[numkilled].equals(this))
                                numkilled = ++numkilled % numheroes;//numkilled = randGen.nextInt(numheroes);
                            ...
                            maxhealth += hero[numkilled].maxhealth / 10;
                            maxstamina += hero[numkilled].maxstamina / 10;
                            maxmana += hero[numkilled].maxmana / 10;
                            strength += hero[numkilled].strength / 10;
                            vitality += hero[numkilled].vitality / 10;
                            intelligence += hero[numkilled].intelligence / 10;
                            health = maxhealth;
                            stamina = maxstamina;
                            mana = maxmana; //restores completely when feeds (still sucks though)
                            setMaxLoad();
                            repaint();
                        }
                    }
                    ...
                    int waterdec = 6;
                    if (DIFFICULTY < 0) {
                        fooddec += DIFFICULTY * 2;
                        waterdec += DIFFICULTY * 2;
                    }
                    food -= fooddec;
                    water -= waterdec;
        ...
        public void stamcheck() {
            ...
            if (p < stamtest) {
                int hit = (int) ((float) maxhealth / 10.0f) + randGen.nextInt(8);
        ...
        public boolean doAbility(String ability, int power, char classgain, Object data) {
            ...
            if (ability.charAt(0) < 'G' || ability.equals("War Cry")) {
                if (ability.equals("Anti-Ven")) {
                    ...
                    try {
                        Spell casting = new Spell(power + "642");
                    ...
                    } //spell# is valid, should be no error...
                    ...
                    try {
                        Spell casting = new Spell(power + "335");
                        for (int j = casting.gain - 1; j >= 0; j--) {
                            casting.powers[j] += randGen.nextInt() % 10 + j * wlevel;
                            if (wlevel == 15) casting.powers[j] += randGen.nextInt(20);
                            if (casting.powers[j] < 1) casting.powers[j] = randGen.nextInt(4) + 1;
                    ...
                    if (weapon == fistfoot || hand == null) {
                        ...
                        if (weapon == fistfoot) {
                            weapon = it;
                            if (it.type == Item.WEAPON || it.type == Item.SHIELD || it.number == 4)
                                it.equipEffect(this);
                            weaponsheet.repaint();
                        } else {
                            hand = it;
                    ...
                    if (randGen.nextInt(16 - nlevel) < 5) {
                        if (DungeonMap[level][xadjust][yadjust].mapchar == '2' || DungeonMap[level][xadjust][yadjust].mapchar == 'i' || (DungeonMap[level][xadjust][yadjust].mapchar == 'p' && ((Pit) DungeonMap[level][xadjust][yadjust]).isIllusionary)) {
                            ...
                            if (DungeonMap[level][xadjust][yadjust].mapchar == 'p')
                                ((Pit) DungeonMap[level][xadjust][yadjust]).isIllusionary = false;
                    ...
                    try {
                        Spell casting = new Spell(power + "52");
                        for (int j = casting.gain - 1; j >= 0; j--) {
                            casting.powers[j] += randGen.nextInt() % 10 + j * wlevel;
                            if (wlevel == 15) casting.powers[j] += randGen.nextInt(20);
                            if (casting.powers[j] < 1) casting.powers[j] = randGen.nextInt(4) + 1;
                    ...
                    abilitywep = weapon;
                    abilityactive = power;
                    if (abilityactive < 1) abilityactive = 1;
                    ...
                    try {
                        Spell casting = new Spell(power + "363");
                        Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                    } catch (Exception e) {
                    } //spell# is valid, should be no error...
                } else if (ability.equals("Fireball")) {
                    try {
                        Spell casting = new Spell(power + "44");
                        for (int j = casting.gain - 1; j >= 0; j--) {
                            casting.powers[j] += randGen.nextInt() % 10 + j * wlevel;
                            if (wlevel == 15) casting.powers[j] += randGen.nextInt(20);
                            if (casting.powers[j] < 1) casting.powers[j] = randGen.nextInt(4) + 1;
                    ...
                    for (int i = 0; i < 6; i++) {
                        ...
                        if (tempmon != null && tempmon.number != 26) {
                            tempmon.timecounter = -power;
                        }
                        if (i == 3) i++;
                    }
                    ...
                    if (ability.equals("War Cry")) {
                        ...
                        else if (avglevel > 12 && avglevel < 15) {
                            adjuster -= 4;
                            power++;
                        } else if (avglevel > 14) {
                            adjuster = 0;
                    ...
                    for (int i = 0; i < 6; i++) {
                        tempmon = (Monster) dmmons.get(level + "," + xadjust + "," + yadjust + "," + i);
                        if (tempmon != null && tempmon.currentai != Monster.FRIENDLYMOVE && tempmon.currentai != Monster.FRIENDLYNOMOVE && randGen.nextInt(11 + adjuster) < tempmon.fearresist) {
                            tempmon.runcounter += power;
                            tempmon.wasfrightened = true;
                        }
                        if (i == 3) i++;
                    }
                }
                ...
                int gain = randGen.nextInt(wlevel / 2) + wlevel / 2 + 1;
                if (gain > 6) gain = 6; //(mana/50+1)%6+1;
                try {
                    Spell casting = new Spell(gain + spls[randGen.nextInt(6)]);
                ...
                darkcounter = 0;
                magictorch += power;
                if (magictorch > 285) magictorch = 285; //can stock up some, but not infinite
                ...
                else if (hand != null && hand.type == Item.FOOD && (hand.poisonous > 0 || hand.foodvalue < 0))
                    it = hand;
                if (it != null) {
                    it.poisonous = 0;
                ...
                try {
                    Spell s1 = new Spell(power + "461");//weakness
                ...
                try {
                    Spell casting = new Spell(power + "523");
                    Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                } catch (Exception e) {
                } //spell# is valid, should be no error...
            } else if (ability.equals("Slow")) {
                try {
                    Spell casting = new Spell(power + "362");
                    Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                } catch (Exception e) {
                } //spell# is valid, should be no error...
                ...
                int[] boost = (int[]) data;
                strength += boost[0];
                strengthboost += boost[0];
                dexterity += boost[1];
                dexterityboost += boost[1];
                vitality += boost[2];
                vitalityboost += boost[2];
                intelligence += boost[3];
                intelligenceboost += boost[3];
                wisdom += boost[4];
                wisdomboost += boost[4];
                if (boost[5] != 0) {
                    if (flevel + boost[5] > 15) {
                        flevelboost += 15 - flevel;
                        flevel = 15;
                    } else if (flevel + boost[5] < 0) {
                        flevelboost += 0 - flevel;
                        flevel = 0;
                    } else {
                        flevelboost += boost[5];
                        flevel += boost[5];
                ...
                if (boost[6] != 0) {
                    if (nlevel + boost[6] > 15) {
                        nlevelboost += 15 - nlevel;
                        nlevel = 15;
                    } else if (nlevel + boost[5] < 0) {
                        nlevelboost += 0 - nlevel;
                        nlevel = 0;
                    } else {
                        nlevelboost += boost[6];
                        nlevel += boost[6];
                ...
                if (boost[7] != 0) {
                    if (wlevel + boost[7] > 15) {
                        wlevelboost += 15 - wlevel;
                        wlevel = 15;
                    } else if (wlevel + boost[5] < 0) {
                        wlevelboost += 0 - wlevel;
                        wlevel = 0;
                    } else {
                        wlevelboost += boost[7];
                        wlevel += boost[7];
                ...
                if (boost[8] != 0) {
                    if (plevel + boost[8] > 15) {
                        plevelboost += 15 - plevel;
                        plevel = 15;
                    } else if (plevel + boost[5] < 0) {
                        plevelboost += 0 - flevel;
                        plevel = 0;
                    } else {
                        plevelboost += boost[8];
                        plevel += boost[8];
                ...
                } else if (tempmon.isImmaterial || tempmon.number == 26)
                    tempmon = null;//can't steal from chaos (#26) - remove if/when have a stealdifficulty int
                if (tempmon != null) {
                    //found a mon, try to steal if has anything, power is chance to succeed
                    if (randGen.nextInt(100) < (power + 3 * nlevel - tempmon.speed / 4)) {
                        if (tempmon.carrying.size() == 0)
                            message.setMessage(tempmon.name + " has nothing to steal.", 4);
                        else {
                ...
                try {
                    Spell casting = new Spell(power + "664");
                    Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                } catch (Exception e) {
                } //spell# is valid, should be no error...
                ...
                try {
                    Spell casting = new Spell(power + "31");
                    Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                } catch (Exception e) {
                } //spell# is valid, should be no error...
            } else if (ability.equals("Venom")) {
                try {
                    Spell casting = new Spell(power + "51");
                    for (int j = casting.gain - 1; j >= 0; j--) {
                        casting.powers[j] += randGen.nextInt() % 10 + j * wlevel;
                        if (wlevel == 15) casting.powers[j] += randGen.nextInt(20);
                        if (casting.powers[j] < 1) casting.powers[j] = randGen.nextInt(4) + 1;
                ...
                try {
                    Spell casting = new Spell(power + "461");
                    Projectile p = new Projectile(casting, casting.dist, facing, projsub);
                } catch (Exception e) {
                } //spell# is valid, should be no error...
            } else if (ability.equals("ZO")) {
                try {
                    Spell casting = new Spell(power + "6");
                    for (int j = casting.gain - 1; j >= 0; j--) {
                        casting.powers[j] += randGen.nextInt() % 10 + j * wlevel;
                        if (wlevel == 15) casting.powers[j] += randGen.nextInt(20);
                        if (casting.powers[j] < 1) casting.powers[j] = randGen.nextInt(4) + 1;
        ...
        public boolean checkmagic(int f) {
            ...
            if (!doAbility(weapon.function[f][0], weapon.power[f], weapon.function[f][1].charAt(0)))
                return false; //do ability, based on function name -> if name not found, is error so just treat as normal attack
        ...
        public boolean checkmon(int f) {
            ...
            if ((subsquare == 2 && heroatsub[1] != -1) || (subsquare == 3 && heroatsub[0] != -1)) {
                if (!DungeonMap[level][xadjust][yadjust].hasMons)
                    return false;//false so will check for a door and say can't reach if is one
                ...
                if (tempmon == null || imtest[who]) {
                    ...
                    if (tempmon == null || imtest[who]) {
                        ...
                        if (tempmon == null || imtest[whocheck] || imtest[who]) {
                            ...
                            tempmon = (Monster) dmmons.get(level + "," + xadjust + "," + yadjust + "," + who);
                            if (imtest[whocheck]) tempmon = null;
                            else if (tempmon != null && ((tempmon.isImmaterial && !weapon.hitsImmaterial) || (!tempmon.isImmaterial && weapon.function[f][0].equals("Disrupt"))))
                                tempmon = null;
                        }
                        //when backstabbing, if chosen mon not facing away try other (only affects 2 furthest subsquares)
                            ...
                            else if (newtemp != null && ((newtemp.isImmaterial && !weapon.hitsImmaterial) || (!newtemp.isImmaterial && weapon.function[f][0].equals("Disrupt"))))
                                newtemp = null;
            ...
            } else if ((tempmon.isImmaterial && !weapon.hitsImmaterial) || (!tempmon.isImmaterial && weapon.function[f][0].equals("Disrupt")))
                tempmon = null;
            ...
            if (tempmon != null) {
                ...
                if (tempmon.subsquare != 5 && (tempmon.subsquare + facing) % 4 < 2) {
                    speeddif -= 10;
                    backhit++;
                }//harder to hit back row
                ...
                if (didhit) {
                    ...
                    if (nlevel > 1 && weapon.function[f][1].equals("n"))
                        pow += (randGen.nextInt(nlevel) + 1) * 3;//was *4
                    ...
                    } else if (dexterity > 50 && randGen.nextInt(20 - 2 * nlevel / 3) == 0) {
                        pow = 3 * pow / 2;
                        message.setMessage(name + ": Critical Hit", heronumber);
                    } //critical hit
                    ...
                    if (weapon.poisonous > tempmon.poisonpow && randGen.nextBoolean()) {
                        tempmon.ispoisoned = true;
                        tempmon.poisonpow = weapon.poisonous;
                    }//50% chance of inflicting poison
                    if (abilitypoison > tempmon.poisonpow && randGen.nextInt(5) > 0) {
                        tempmon.ispoisoned = true;
                        tempmon.poisonpow = abilitypoison;
                    }//4/5 chance of inflicting poison from an ability
                    ...
                    if ((abilitystun || weapon.function[f][0].equals("Stun")) && tempmon.number != 26 && tempmon.health < 2 * tempmon.maxhealth / 5 && randGen.nextBoolean()) {
                        tempmon.timecounter = -weapon.power[f];
                        if (tempmon.timecounter > -10) tempmon.timecounter = -10;
                        if (nlevel > 8 && !tempmon.isdying)
                            message.setMessage(name + "'s blow has stunned the " + tempmon.name + ".", heronumber);
                    }
        ...
        public void checkdoor(int f) {
            if (weapon.number == 9 || (weapon.number > 235 && weapon.number < 300) || !(weapon.function[f][0].equals("Swing") || weapon.function[f][0].equals("Chop") || weapon.function[f][0].equals("Melee") || weapon.function[f][0].equals("Bash") || weapon.function[f][0].equals("Crush") || weapon.function[f][0].equals("Berzerk")))
                return;
            if ((subsquare == 2 && heroatsub[1] != -1) || (subsquare == 3 && heroatsub[0] != -1)) {
                message.setMessage(name + " can't reach.", 4);
                //weaponsheet.hitlabel.setText("Can't Reach");
        ...
        public void wepThrow(Item wep, int subsq) {
            ...
            if (nlevel > 0 && (wep.projtype > 0 || (wep.type == Item.WEAPON && wep.hasthrowpic)))
                wep.shotpow += randGen.nextInt(nlevel) + nlevel;
            Projectile p;
            if (wep.isbomb) {
                try {
                    Spell tempspell = new Spell(wep.bombnum);
                    ...
                    p = new Projectile(wep, dist, facing, subsq);
                }
        ...
        public int castSpell() {
            ...
            Spell casting;
            try {
                casting = new Spell(currentspell);
            } catch (Exception e) {
                return 0;
            }
            
            if (casting.gain == 6 && casting.number == 3 && (hand == null || weapon == null || hand.number == weapon.number || (hand.number != 83 && hand.number != 285) || (weapon.number != 285 && weapon.number != 83)))
                return 0;
            ...
            if (attempt >= spelldif) {
                ...
                if (casting.clsgain == 'w' && casting.type != 2 && casting.number != 461 && casting.number != 363 && casting.number != 362 && casting.number != 664 && casting.number != 523) {
                    ...
                    if (casting.type != 0 || casting.number == 1 || casting.number == 2 || casting.number == 655) {
                        ...
                        if (casting.power < 1) casting.power = randGen.nextInt(4) + 1;
                        if (casting.power < 4 && (casting.number == 1 || casting.number == 2 || casting.number == 655))
                            casting.power = randGen.nextInt(4) + 4;
        ...
        public int specialSpell(Spell s) {
            ...
            if (s.number == 325) {
                magicvision += s.power;
                if (magicvision == s.power) needredraw = true;
            }
            //magic torch
            else if (s.number == 4) {
                //darkcounter=0;
            ...
            //zo kath ra
                ...
                if (weapon.name.equals("Fist/Foot")) {
                    ...
                    DungeonMap[level][partyx][partyy].addItem(new Item(80));
                    needredraw = true;
        ...
        public int damage(int ht, int type) {
            if (!isdead) {
                ...
                if (type == WEAPONHIT || type == PROJWEAPONHIT) {
                    ...
                    if (hashurt) {
                        if (hurthand && hand != null && hand.type == Item.SHIELD && hand.defense > 0)
                            defense -= hand.defense;
                        if (hurtweapon && weapon != null && weapon.type == Item.SHIELD && weapon.defense > 0)
                    ...
                    if (hashurt) {
                        if (hurthand && hand != null && hand.type == Item.SHIELD && hand.magicresist > 0)
                            magicresist -= hand.magicresist;
                        if (hurtweapon && weapon != null && weapon.type == Item.SHIELD && weapon.magicresist > 0)
                            magicresist -= weapon.magicresist;
                ...
                if (health < 1) {
                    ...
                    hurtcounter = 3;
                    repaint();
                    if (sheet && !viewing && !sleeping) herosheet.repaint();
        ...
        public void paint(Graphics gg) {
            if (!isdead) {
                ...
                g.fillRect(0, 0, this.getSize().width, 84);
                g.setColor(new Color(150, 150, 150));
                g.fillRect(0, 0, this.getSize().width, 18);
                if (hurtcounter > 0) {
                ...
                g.fillRect(this.getSize().width - 10, 0, 10, 10);
